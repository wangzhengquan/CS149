...
extern void saxpySerial(int N, float a, float* X, float* result);
...
int main() {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        saxpySerial(N, scale, arrayX, resultSerial);
...
printf("[saxpy serial]:\t\t[%.3f] ms\t[%.3f] GB/s\t[%.3f] GFLOPS\n",
    ...
    for (int i = 0; i < 3; ++i) {
        double startTime = CycleTimer::currentSeconds();
        saxpy_ispc(N, scale, arrayX, resultISPC);
        double endTime = CycleTimer::currentSeconds();
    ...
    for (int i = 0; i < 3; ++i) {
        double startTime = CycleTimer::currentSeconds();
        saxpy_ispc_withtasks(N, scale, arrayX, resultTasks);
        double endTime = CycleTimer::currentSeconds();
