...
void absVector2(float* values, float* output, int N) {
  __cs149_vec_float result;
  __cs149_vec_float zero = _cs149_vset_float(0.f);
  __cs149_mask maskAll, maskIsNegative;
...
//  Why is that the case?
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    // Load vector of values from contiguous memory addresses
    _cs149_vload_float(result, values+i, maskAll);               // x = values[i];

    // Set mask according to predicate
    _cs149_vlt_float(maskIsNegative, result, zero, maskAll);     // if (x < 0) {

    // Execute instruction using mask ("if" clause)
    _cs149_vsub_float(result, zero, result, maskIsNegative);      //   output[i] = -x;
...
void clampedExpVector1(float* values, int* exponents, float* output, int N) {
  //
  ...
  //
  __cs149_vec_float x, result, max_val;
  __cs149_vec_int exp;
  __cs149_mask maskAll;
  int i;

  // Initialize max_val vector to 9.999
  max_val = _cs149_vset_float(9.999999f);
  ...
  for (i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    // Load vectors from contiguous memory addresses
    _cs149_vload_float(x, values+i, maskAll);
    _cs149_vload_int(exp, exponents+i, maskAll);

    // Loop over each element of vector x and exp
    for(int j=0; j<VECTOR_WIDTH; j++) {
        float cur_val = x.value[j];
        int cur_exp = exp.value[j];
        if(cur_exp == 0){
          result.value[j] = 1;
        } else{
          // Compute clamped value
          float clamped_val = cur_val;
          for(int k=0; k<cur_exp-1; k++) {
              clamped_val *= cur_val;
          }
          clamped_val = (clamped_val > 9.999999f) ? 9.999999f : clamped_val;

          // Write clamped value to output vector
          result.value[j] = clamped_val;
       }
    }
...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    _cs149_vgt_float(clampMask, result, max_val, maskAll);
...
float arraySumVector(float* values, int N) {
  __cs149_vec_float result, sum;
  __cs149_mask maskAll;
  
  maskAll = _cs149_init_ones();
  // Initialize sum vector to all zeros
  sum = _cs149_vset_float(0.0f);
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    // Load vector from contiguous memory addresses
    _cs149_vload_float(result, values+i, maskAll);
    // Add vector to running sum
    _cs149_vadd_float(sum, sum, result, maskAll);
  }

  // Sum all elements of vector
  // float finalSum = 0.0f;
  // for (int j=0; j<VECTOR_WIDTH; j++) {
  //   finalSum += sum.value[j];
  // }
